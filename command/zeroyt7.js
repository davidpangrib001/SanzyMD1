...
case 'ytmp33': {
  if (!text) throw `Example : ${prefix + command} https://youtube.com/watch?v=PtFMh6Tccag%27`
                ...
                let { dl_link, thumb, title, filesizeF, filesize } = result
                if (Number(filesize) >= 40000) return m.reply('File Melebihi Batas'+util.format(result))
                zeroyt7.sendImage(m.chat, thumb, `⭔ Title : ${title}\n⭔ File Size : ${filesizeF}\n⭔ Url : ${text}\n⭔ Ext : MP3`, m)
                zeroyt7.sendMessage(m.chat, { audio: { url: dl_link }, mimetype: 'audio/mp4' }, { quoted: m })
            }
            break
case 'play': {
if (!text) throw `Example : ${prefix + command} story wa anime`
                ...
                let buttons = [
                    {buttonId: `ytmp33 ${anu.url} 128kbps`, buttonText: {displayText: '♫ Audio'}, type: 1},
                    {buttonId: `ytmp44 ${anu.url} 360p`, buttonText: {displayText: '► Video'}, type: 1}
                ]
                let buttonMessage = {
